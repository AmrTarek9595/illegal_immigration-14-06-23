...
use App\Models\table_information;
use Illuminate\Support\Facades\Validator;
use App\Models\profit_company_information;
use App\Models\profit_table_information;

class amr_test extends Controller
{

/**admin add non-profit data API    */

public function adddatanonprofit(Request $request){
    
    $row=new company_information;
    $row->Gov=$request->GOV;
...
public function displayallprofit(){
    $new= profit_company_information::get();
...
public function profit_insert_table_information(Request $request)
{
      ...
      $table=new profit_table_information;
...
public function retrive_datasohag(Request $request){
    /*  $query= company_information::where('Gov', $request->gov)
        ->Orwhere('markaz',$request->selectedasuit)
        ->Orwhere('markaz',$request->selectedsohag)
        ->Orwhere('karya',$request->asuitkarya)
        ->Orwhere('karya',$request->sohagkarya)
    ...
    if($request->gov == "sohag" && $request->markaz == 'akhmem' )
    {
        ...
        {
        $query= company_information::Where('karya','=',$request->karya)
        ->where('Gov','=',$request->gov)
        ->where('markaz','=',$request->markaz)
        ->get();
        return response()->json(['message'=>$query]);
        }
        else if($request->gov == "sohag" && $request->markaz == 'akhmem' && $request->karya == "0"){
            $query= company_information::where('Gov','=',$request->gov)
        ->where('markaz','=',$request->markaz)
        ->get();
        return response()->json(['message'=>$query]);
...
public function retrive_customdata_nonprofit(Request $request){
    ...
    $query= company_information::where('id',$request->id)->get();
...
public function retrive_customdata_nonprofitinformationtable(Request $request){
    ...
    $query= table_information::where('keys',$request->id)->get();
...
public function retrive_datasohagprofit(Request $request){
    ...
    if($request->gov == "sohag" && $request->markaz == 'akhmem' )
    {
        if($request->gov == "sohag" && $request->markaz == 'akhmem' && $request->karya !== "0")
        {
        $query= profit_company_information::Where('karya','=',$request->karya)
        ->where('Gov','=',$request->gov)
        ->where('markaz','=',$request->markaz)
        ->paginate(6);
        return response()->json(['message'=>$query]);
            ...
            $query= profit_company_information::where('Gov', $request->gov)
        ->where('markaz',$request->markaz)
    ...
    $query= profit_company_information::where('Gov', $request->gov)->orderBy('name_of_company')
        ->paginate(6);
    return response()->json(['message'=>$query]);
...
public function retrive_customdata_profit(Request $request){
    ...
    $query= profit_company_information::where('id',$request->id)->get();
...
public function retrive_customdata_profit_informationtable(Request $request){
    $query=profit_table_information::where('keys',$request->id)->get();
    return response()->json(['message'=>$query]);
}
